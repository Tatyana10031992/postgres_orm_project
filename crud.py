...
def get_total_active_projects_budget():
       with SessionLocal() as session:
            stmt = select(func.sum(Project.budget).where(Project.is_active(True)))
            return session.execute(stmt).scalar()
       
def count_employees_by_department():
      with SessionLocal() as session:
           stmt = select(Department.name, func.count(Employee.id).join(Employee, Employee.department_id == Department.id).grouo_by(Department.name))
           return session.execute(stmt).all()
...
def deativate_project(project_id: int):
       with SessionLocal() as session:
            project = session.get(Project, project_id)
